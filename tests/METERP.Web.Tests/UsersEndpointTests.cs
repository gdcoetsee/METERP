...
public class UsersEndpointTests : IClassFixture<MeterpWebApplicationFactory>
{
    private readonly MeterpWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public UsersEndpointTests(MeterpWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task Users_RedirectsToLogin_WhenUnauthenticated()
    {
    ...
    [Fact]
    public async Task Users_ReturnsOk_WhenAuthenticated()
    {
        const string email = "users@acme.demo";
        await EnsureUsersUserAsync(email);
        ...
        Assert.Contains("Users", body, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task Users_IsNotRateLimited_UnderBurst()
    {
        for (var i = 0; i < 35; i++)
        {
            var response = await _client.GetAsync("/users");
    ...
    private async Task EnsureUsersUserAsync(string email)
    {
        ...
        await userManager.AddClaimAsync(user, new Claim("Permission", Permissions.UsersView));
    }
}