...
public class AiSettingsEndpointTests : IClassFixture<MeterpWebApplicationFactory>
{
    private readonly MeterpWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public AiSettingsEndpointTests(MeterpWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task AiSettings_RedirectsToLogin_WhenUnauthenticated()
    {
        var response = await _client.GetAsync("/settings/ai");
    ...
    [Fact]
    public async Task AiSettings_ReturnsOk_WhenAuthenticated()
    {
        const string email = "aisettings@acme.demo";
        await EnsureAiSettingsUserAsync(email);
    ...
    [Fact]
    public async Task AiSettings_IsNotRateLimited_UnderBurst()
    {
        for (var i = 0; i < 35; i++)
        {
            var response = await _client.GetAsync("/settings/ai");
    ...
    private async Task EnsureAiSettingsUserAsync(string email)
    {
        ...
        await userManager.AddClaimAsync(user, new Claim("Permission", Permissions.TenantsManage));
    }
}