...
public class LogoutEndpointTests : IClassFixture<MeterpWebApplicationFactory>
{
    private readonly MeterpWebApplicationFactory _factory;

    public LogoutEndpointTests(MeterpWebApplicationFactory factory) => _factory = factory;

    [Fact]
    public async Task Logout_RedirectsToHome_WhenAuthenticated()
    {
        const string email = "logout-home@acme.demo";
    ...
    [Fact]
    public async Task Logout_ClearsSession_ForProtectedRoutes()
    {
        const string email = "logout-session@acme.demo";
        await EnsureUserAsync(email);
        ...
        Assert.Contains("account-hub-ready", beforeBody, StringComparison.OrdinalIgnoreCase);

        await client.GetAsync("/logout");

        var after = await client.GetAsync("/account");
        Assert.Equal(HttpStatusCode.Redirect, after.StatusCode);
        Assert.Contains("/login", after.Headers.Location?.ToString(), StringComparison.OrdinalIgnoreCase);
    }
    ...
    private async Task EnsureUserAsync(string email)
    {
        ...
        if (tenantId == Guid.Empty)
        {
            ...
            db.Tenants.Add(new Tenant
            {
                Id = tenantId,
                TenantId = tenantId,
                Name = "Logout Test Tenant",
                Subdomain = "logouttest",
                IsActive = true
            });
