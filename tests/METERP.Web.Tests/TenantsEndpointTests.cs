...
public class TenantsEndpointTests : IClassFixture<MeterpWebApplicationFactory>
{
    private readonly MeterpWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public TenantsEndpointTests(MeterpWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task Tenants_RedirectsToLogin_WhenUnauthenticated()
    {
    ...
    [Fact]
    public async Task Tenants_ReturnsOk_WhenAuthenticated()
    {
        const string email = "tenants@acme.demo";
        await EnsureTenantsUserAsync(email);
        ...
        Assert.Contains("tenants-ready", body, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task Tenants_IsNotRateLimited_UnderBurst()
    {
        for (var i = 0; i < 35; i++)
        {
            var response = await _client.GetAsync("/tenants");
    ...
    private async Task EnsureTenantsUserAsync(string email)
    {
        ...
        await userManager.AddClaimAsync(user, new Claim("Permission", Permissions.TenantsView));
    }
}