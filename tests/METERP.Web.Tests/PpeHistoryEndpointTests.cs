...
public class PpeHistoryEndpointTests : IClassFixture<MeterpWebApplicationFactory>
{
    private readonly MeterpWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public PpeHistoryEndpointTests(MeterpWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task PpeHistory_RedirectsToLogin_WhenUnauthenticated()
    {
        var response = await _client.GetAsync("/ppe-history");
    ...
    [Fact]
    public async Task PpeHistory_ReturnsOk_WhenAuthenticated()
    {
        const string email = "ppehistory@acme.demo";
        await EnsurePpeHistoryUserAsync(email);
        ...
        Assert.True(
            ...
            || body.Contains("ppe-history-empty", StringComparison.OrdinalIgnoreCase)
            || body.Contains("ppe-history-skeleton", StringComparison.OrdinalIgnoreCase),
            "Expected PPE history table, empty, or loading marker.");
    }

    [Fact]
    public async Task PpeHistory_IsNotRateLimited_UnderBurst()
    {
        for (var i = 0; i < 35; i++)
        {
            var response = await _client.GetAsync("/ppe-history");
    ...
    private async Task EnsurePpeHistoryUserAsync(string email)
    {
