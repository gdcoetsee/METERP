...
public class PayrollEndpointTests : IClassFixture<MeterpWebApplicationFactory>
{
    private readonly MeterpWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public PayrollEndpointTests(MeterpWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task Payroll_RedirectsToLogin_WhenUnauthenticated()
    {
        var response = await _client.GetAsync("/payroll");
    ...
    [Fact]
    public async Task Payroll_ReturnsOk_WhenAuthenticated()
    {
        const string email = "payroll@acme.demo";
        await EnsurePayrollUserAsync(email);
    ...
    [Fact]
    public async Task Payroll_IsNotRateLimited_UnderBurst()
    {
        for (var i = 0; i < 35; i++)
        {
            var response = await _client.GetAsync("/payroll");
    ...
    private async Task EnsurePayrollUserAsync(string email)
    {
