...
public class AuditEndpointTests : IClassFixture<MeterpWebApplicationFactory>
{
    private readonly MeterpWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public AuditEndpointTests(MeterpWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task Audit_RedirectsToLogin_WhenUnauthenticated()
    {
    ...
    [Fact]
    public async Task Audit_ReturnsOk_WhenAuthenticated()
    {
        const string email = "audit@acme.demo";
        await EnsureAuditUserAsync(email);
        ...
        Assert.Contains("audit-ready", body, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("audit-export-csv", body, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task Audit_IsNotRateLimited_UnderBurst()
    {
        for (var i = 0; i < 35; i++)
        {
            var response = await _client.GetAsync("/audit");
    ...
    private async Task EnsureAuditUserAsync(string email)
    {
        ...
        await userManager.AddClaimAsync(user, new Claim("Permission", Permissions.AuditView));
    }
}